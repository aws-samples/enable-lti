...
describe('jwks.json Tests', () => {
  ...
  it('Found JWK, TTL < 10 days', async () => {
    ...
    ddbMock
      .on(GetItemCommand)
      .resolvesOnce(entry)
      .resolves(refreshEntry)
      .on(PutItemCommand)
      .resolves(data.jwksPutItem);
    ...
    expect(response.body).toBeDefined();
    expect(response.statusCode).toBe(200);
    expect(JSON.parse(response.body).keys[0].kid).toBe(
      refreshEntry.Item!['kid']['S']
    );
  });
  ...
  it('Not found JWK', async () => {
    ...
    ddbMock
      .on(GetItemCommand)
      .resolvesOnce(data.jwksEmptyEntry)
      .resolves(refreshEntry)
      .on(PutItemCommand)
      .resolves(data.jwksPutItem);
    kmsMock.on(GetPublicKeyCommand).resolves(data.jwksGetPublicKey);
