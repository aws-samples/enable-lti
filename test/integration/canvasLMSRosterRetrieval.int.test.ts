import { handler as rosterRetrievalHandler } from '@enable-lti/roster-retrieval/src';
import {
  DynamoDBJwks,
...
import { platformConfig, jwtBodyForRosterRetrieval } from '../utils/models';
...
describe('CanvasLMS login launch flow works', () => {
  ...
  test('user launched tool from CanvasLMS, OIDC launch flow', async () => {
    ...
    const retrievalEvent = rosterRetrievalRequestEvent(signedJWT);
